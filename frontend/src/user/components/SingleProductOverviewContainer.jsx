...
import "./SingleProductOverviewContainer.css";

function SingleProductOverviewContainer({ title, content }) {
  const [isContainerExpanded, setIsContainerExpanded] = useState(false);

  return (
    <div className={`product-${title.toLowerCase()}-container`}>
      <div
        className={`${title.toLowerCase()}-header`}
        onClick={() => setIsContainerExpanded((prevValue) => !prevValue)}
      >
        <h2>{title}</h2>
        {isContainerExpanded ? (
          <svg
      ...
      {isContainerExpanded && <>{content}</>}
    </div>
  );
}

export default SingleProductOverviewContainer;
